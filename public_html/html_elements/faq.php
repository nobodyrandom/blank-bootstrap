
<!--

This module displays faqs, providing a place for large amounts of hideable text.

-->

<div id="faq"">
    <div class="container">
        <div class="row faqContainer">

            <div class="col-sm-10 col-sm-offset-1">

                <!-- The header of the faq, toggles it's associated href -->
                <a href="#faq1"  data-toggle="collapse"><h2 class="faqHeader">FAQ #1</h2><hr style="width: 100%"><br></a>

                <!-- The content of the faq -->
                <div id="faq1" class="collapse">
                    <p style="font-size: 14px;">
                        Lorem ipsum dolor sit amet, ne quas soleat interpretaris mel.
                        Cu quo vide eruditi iracundia, has eu feugait noluisse. Ad
                        amet recteque laboramus vim, sea nominavi legendos at. Per ut
                        ullum imperdiet, fierent mediocrem ad sit.
                    </p>
                </div>
            </div>

            <div class="col-sm-10 col-sm-offset-1">
                <a href="#faq2"  data-toggle="collapse"><h2 class="faqHeader">FAQ #2</h2><hr style="width: 100%"><br></a>
                <div id="faq2" class="collapse">
                    <p style="font-size: 14px;">
                        Lorem ipsum dolor sit amet, ne quas soleat interpretaris mel.
                        Cu quo vide eruditi iracundia, has eu feugait noluisse. Ad
                        amet recteque laboramus vim, sea nominavi legendos at. Per ut
                        ullum imperdiet, fierent mediocrem ad sit.
                    </p>
                </div>
            </div>
            <div class="col-sm-10 col-sm-offset-1">
                <a href="#faq3"  data-toggle="collapse"><h2 class="faqHeader">FAQ #3</h2><hr style="width: 100%"><br></a>
                <div id="faq3" class="collapse">
                    <p style="font-size: 14px;">
                        Lorem ipsum dolor sit amet, ne quas soleat interpretaris mel.
                        Cu quo vide eruditi iracundia, has eu feugait noluisse. Ad
                        amet recteque laboramus vim, sea nominavi legendos at. Per ut
                        ullum imperdiet, fierent mediocrem ad sit.
                    </p>
                </div>
            </div>
        </div>
    </div>
</div>